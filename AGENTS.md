...
15. Every material technical route must preserve an explicit continuation path into the next expected development stage. Do not choose a host-specific or launch-size shortcut that predictably forces a near-term rewrite, authority-model change, or avoidable migration. Keep scale-sensitive values as tuning parameters where practical; do not overbuild speculative future functionality. Apply `governance/ENGINEERING_CONTINUITY_AND_SCALE_PROVIDER_GATE_V1_2026-08-16.md`.
16. Before first deployment or any material change to market count, historical depth, scan cadence, provider/API usage, confirmation/retry count, concurrency, database workload, or runtime cohort size, run the lightweight Scale and Provider Budget Gate plus a realistic-size acceptance test. Any material Universe-size change requires renewed Provider Budget + Freshness qualification before release.
