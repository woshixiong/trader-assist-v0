...
Reviewer runtime is delta-first and one-pass by default. Start from the exact Review Manifest, then exact target/diff, exact-head CI summary, decisive evidence, and only the targeted authority/source reads needed for unresolved acceptance questions. Do not use broad discovery searches, repeated reads of unchanged artifacts, raw long logs or large full-file reads by default. If interrupted, preserve the exact review checkpoint/result key and remaining questions; resume from that checkpoint without requiring the user to type repeated "continue" messages or restarting discovery.
