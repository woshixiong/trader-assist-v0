...
TRIAL_AND_ADAPTIVITY_LEDGER_STATUS=PASS|FAIL|NOT_APPLICABLE
...
For trading Strategy work where material complexity is proposed, additionally require:
...
MATERIAL_VARIANT_COUNT_OR_SEARCH_HISTORY_RECORDED=YES|NO|NOT_APPLICABLE
...
For a material research/data task triggered by a research frontier, `ENGINEERING_PREFLIGHT_GATE=PASS` additionally requires an explicit `RESEARCH_INVESTMENT_DISPOSITION`. No data/research Writer may infer `ACQUIRE_BEFORE_NEXT_GATE` from `DATA_BLOCKED_FRONTIER` alone.
