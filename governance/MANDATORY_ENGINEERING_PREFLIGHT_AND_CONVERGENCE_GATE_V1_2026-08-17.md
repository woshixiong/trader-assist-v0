...
### 1D. Universal execution classification and pre-model semantic launch gate

Engineering Control / Router must resolve the execution class before Writer dispatch. The classification selects the execution surface only; it does not alter the existing engineering state machine or any acceptance/authority gate.

```text
EXECUTION_CLASS=DETERMINISTIC_MECHANICAL
=> MODEL_WRITER_REQUIRED=NO
=> EXECUTION_SURFACE_CLASS=GITHUB_DETERMINISTIC|LOCAL_DETERMINISTIC|ENGINEERING_CONTROL_DIRECT

EXECUTION_CLASS=FROZEN_BOUNDED_SEMANTIC
...
=> DECISIVE_ACCEPTANCE_TESTS_SPECIFIED=YES
=> NEW_ROOT_CAUSE_OR_SCOPE_EXPANSION_STOP=YES
=> EXECUTION_SURFACE_CLASS=ORDINARY_CHATGPT_WRITER by routine default

EXECUTION_CLASS=OPEN_MATERIAL_SEMANTIC
=> CODEX_DEFAULT_WHEN_AVAILABLE_AND_ALLOWED=YES

EXECUTION_CLASS=HIGH_CONSEQUENCE_AMBIGUOUS
=> STRONGEST_APPROPRIATE_ACCEPTED_ROUTE=REQUIRED
```

For any Codex Desktop/local managed-worktree launch, record the controller/runner-produced pre-model proof **before semantic model invocation**:

```text
CODEX_MANAGED_WORKTREE_APPLICABLE=YES|NO
PRE_MODEL_DETERMINISTIC_BASE_GATE=PASS|FAIL|NOT_APPLICABLE
CANONICAL_REMOTE_TRACKING_REF=
REMOTE_TRACKING_REF_FRESHENED=YES|NO|NOT_APPLICABLE
REMOTE_TRACKING_REF_EQ_FROZEN_EXACT_BASE=YES|NO|NOT_APPLICABLE
BOUND_EXACT_TREE_VERIFIED=YES|NO|NOT_APPLICABLE
MANAGED_WORKTREE_CREATED_OR_REALIGNED_FROM_EXACT_BASE=YES|NO|NOT_APPLICABLE
MANAGED_WORKTREE_HEAD_EQ_FROZEN_EXACT_BASE=YES|NO|NOT_APPLICABLE
MANAGED_WORKTREE_CLEAN=YES|NO|NOT_APPLICABLE
SEMANTIC_MODEL_START_ALLOWED=YES|NO
```

Hard enforcement:

```text
CODEX_MANAGED_WORKTREE_APPLICABLE=YES
AND PRE_MODEL_DETERMINISTIC_BASE_GATE != PASS
=> SEMANTIC_MODEL_START_ALLOWED=NO
=> CODEX_SEMANTIC_MODEL_INVOCATION=PROHIBITED

DIRTY_OR_AMBIGUOUS_MANAGED_WORKTREE
=> DETERMINISTIC_REALIGNMENT=PROHIBITED
=> RETURN_TO_ENGINEERING_CONTROL

CLEAN_WORKTREE
+ FRESH_REMOTE_TRACKING_REF_EQ_FROZEN_EXACT_BASE
+ BOUND_EXACT_TREE_VERIFIED_WHEN_APPLICABLE
=> EXACT_BASE_REALIGNMENT_BEFORE_MODEL_START=EXECUTION_SURFACE_RECOVERY
=> APPLICATION_SEMANTIC_REPAIR_CONSUMED=NO
```

A stale local branch label is never proof of canonical main. The Writer-side preflight may consume this proof but cannot substitute a prompt reminder for the controller/runner gate.
...
TERMINAL_RESULT_FRESH_READ_REQUIRED=YES|NO|NOT_APPLICABLE
TERMINAL_RESULT_CANONICAL_STATE_LOCATOR=
TERMINAL_RESULT_CANONICAL_STATE_FRESH=YES|NO|NOT_APPLICABLE
TERMINAL_RESULT_RECONCILIATION=NOT_REQUIRED|IDEMPOTENT_RECONCILIATION_REQUIRED|NOT_APPLICABLE
```
...
Before a Writer / Reviewer / executor writes any terminal result or blocker:

```text
TERMINAL_RESULT_FRESH_READ_REQUIRED=YES
TERMINAL_RESULT_CANONICAL_STATE_FRESH=YES
IF_CANONICAL_STATE_ALREADY_ADVANCED_EXTERNALLY
=> TERMINAL_RESULT_RECONCILIATION=IDEMPOTENT_RECONCILIATION_REQUIRED
=> STALE_BLOCKER_OR_STALE_TERMINAL_STATE_WRITE=PROHIBITED
```
...
CI_FAILURE_CLASS=TRANSIENT_OR_KNOWN_FLAKE
AND SAME_HEAD_RERUN_COUNT>=1
AND CI_STILL_FAILS
=> ANOTHER_BLIND_RERUN=PROHIBITED
...
=> APPLICATION_SEMANTIC_REPAIR_CONSUMED=NO
=> NEW_HEAD_EXACT_HEAD_CI_REQUIRED=YES

CI_FAILURE_CLASS=SEMANTIC
=> APPLICATION_SEMANTIC_REPAIR_CONSUMED=YES
=> FROZEN_REPAIR_BUDGET_APPLIES=YES

CI_FAILURE_CLASS=INFRASTRUCTURE_OR_TRANSPORT
=> SEMANTIC_WRITER_REDISPATCH=PROHIBITED_BY_DEFAULT
=> APPLICATION_SEMANTIC_REPAIR_CONSUMED=NO

CI_FAILURE_CLASS=UNRESOLVED
=> RETRY_LOOP=PROHIBITED
=> CODE_MUTATION=PROHIBITED
=> MINIMUM_DECISIVE_EVIDENCE_REQUIRED=YES
```

Review findings do not have a separate executor-routing taxonomy:

```text
REVIEW_FAIL
-> ENGINEERING_CONTROL_CREATES_NEW_BOUNDED_DEVELOPMENT_TASK
-> RUN_UNIVERSAL_EXECUTION_CLASSIFICATION
-> SELECT_ROUTE_FROM_NEW_TASK_CLASS
```

A new root cause, authority drift, architectural ambiguity or write-surface expansion returns to Engineering Control and requires the affected Task Packet/preflight to be re-frozen before Writer dispatch.
...
`ENGINEERING_PREFLIGHT_GATE=PASS` only when every applicable field above is resolved, `EXECUTION_CLASS` and its rationale are frozen, and the selected Writer/executor has the capabilities required to perform the frozen task and its mandatory gates. A `FROZEN_BOUNDED_SEMANTIC` task may not PASS unless all bounded-semantic predicates in §1D are satisfied. An applicable Codex managed-worktree launch may not PASS semantic-start readiness unless `PRE_MODEL_DETERMINISTIC_BASE_GATE=PASS`. This includes `STAGE_CLAIM_LADDER`, `AUTHORITATIVE_PROOF_SURFACE_FOR_EACH_STAGE`, `MINIMUM_REPRESENTATIVE_TOPOLOGY`, `COMPLEXITY_ESCALATION_ORDER`, `PRE_WRITER_GO_NO_GO_EXPERIMENT`, `DETERMINISTIC_BRANCH_PROOF_PLAN` and `REAL_EXTERNAL_BOUNDARY_PROOF_PLAN` when applicable.
