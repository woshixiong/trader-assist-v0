...
OFFICIAL_RATE_LIMIT_AUTHORITY_HASH: f6450a7a81246751c4de74de466af5fcaab7265c45950036fb5d7a9db9377715
...
R2 independently read the query-free canonical rate-limit page twice at `2026-07-12T11:46:08Z` and `2026-07-12T11:46:09Z`, with the second request using no-cache headers. Both returned HTTP 200 and matching normalized official semantics: 10 simultaneous WebSocket connections, 30 new connections per minute, and the documented previous-day maker-share / once-per-UTC-date wording remain frozen. The supporting Info page returned HTTP 200 at `2026-07-12T11:49:21Z`. The UI showed only relative update observations, so the authoritative marker is `None`. Retrieval timestamps are metadata, not publication or effective-revision times.
...
The public `authenticate_official_rate_limit_authority_json` boundary accepts only exact raw `str` or `bytes`, rejects duplicate keys before information can be lost, requires exact built-in JSON primitive types and exact frozen membership/order, reconstructs every hash, and returns an immutable non-Pydantic result. Internally, consumers authenticate only repository-controlled exact primitive mappings. Pydantic models, builders, Schema-valid objects, TypeAdapter/core results, and the authentication result itself are never accepted as authority input. The generated JSON Schema proves structure only and cannot authenticate source identity, membership/order, conflicts, evidence, hashes, or transition authority.
