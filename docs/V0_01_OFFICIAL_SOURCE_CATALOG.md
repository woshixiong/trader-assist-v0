...
The query-free rate-limit page was independently retrieved at `2026-07-12T11:46:08Z` and, with no-cache headers, at `2026-07-12T11:46:09Z`. Both reads returned HTTP 200 and the same normalized body semantics. The supporting query-free Info page returned HTTP 200 at `2026-07-12T11:49:21Z`. The public UI exposed only relative observations (`2 months ago` and `1 month ago` respectively), so `publication_or_last_updated_marker` is `None`; relative UI text is not an independently confirmable publication or effective-revision timestamp. Retrieval times are observation metadata, and the legacy source-catalog verification date remains separately frozen at `2026-07-07`.
...
`OfficialRateLimitAuthorityV0` and its nested Pydantic models are untrusted structural, serialization, and Schema containers. Builders and helpers return candidates, not authenticated authority, and low-level Pydantic APIs may construct counterfeit instances. Final authenticity never follows from Python instance identity. Wire or stored material must enter through `authenticate_official_rate_limit_authority_json` as exact `str` or `bytes`, where strict UTF-8 decoding, duplicate-key rejection at every depth, exact primitive typing, exact membership/order, semantic checks, and all hashes are reauthenticated. Repository-controlled primitive construction uses a private exact-dict/list authenticator. Every actual consumer follows private primitive builder → private mapping authenticator → immutable non-Pydantic authentication result; it never trusts a Pydantic instance.
