...
2. Require a controller/runner-produced **pre-model deterministic base-freshness proof** before semantic work: fresh canonical `origin/main` (when applicable) equals the frozen exact base; bound expected tree matches when supplied; managed-worktree HEAD equals the frozen exact base; worktree is clean. A stale local `main` label is not evidence. Missing/mismatched proof => stop before semantic mutation and return deterministic execution-surface drift to control.
